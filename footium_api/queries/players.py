...
def get_players_in_clubs(gql: GqlConnection, club_ids: List[str]):
    filter = {
        "clubId": {"in": club_ids},
    }
...
"""
    ...
    players['clubName'] = players['clubName'].apply(lambda x: x['name'] if x is not None else None)
    players['clubOwnerId'] = players['clubOwnerId'].apply(lambda x: x['ownerId'] if x is not None else None)

    # unwrap playerAttributes
    attributes_list = [attr[0] for attr in players['playerAttributes'] if attr] 
    combined_data_with_ids = [
        {**attr, 'id': player_id} for attr, player_id in zip(attributes_list, players.index)
    ]
    ...
    conflicting_columns = {col for col in attributes_df.columns if col in existing_columns}
    ...
    combined_data_with_ids = [
        {**pos_dict, 'id': player_id} for pos_dict, player_id in zip(positional_rating_dict_list, players.index)
    ]
    ...
    existing_columns = set(players.columns)
    conflicting_columns = {col for col in positional_rating_df.columns if col in existing_columns}
    for col in conflicting_columns:
        positional_rating_df.rename(columns={col: 'positionalRating_' + col}, inplace=True)
    ...
    # fix types
    players['assetId'] = pd.to_numeric(players['assetId'], errors='coerce').astype('Int64')
    all_positions = ["CF", "LW", "RW", "AM", "LM", "CM", "RM", "DM", "LWB", "RWB", "LB", "CB", "RB", "GK"]
