...
def get_formations(gql: GqlConnection):
    query = """
query Formations {
    formations {
        id
...
def convert_formations_to_pd(formations_input):
    ...
    for formation in formations_input:
        ...
        assert sum(value for key, value in row.items() if key in all_positions) == 11, "Total positions do not sum up to 11"
