...
def get_next_fixtures(gql: GqlConnection, club_ids: List[int], max_games:int = None):
    query = """
        ...
        for clubFixture in fixture['clubFixtures']:
            if clubFixture['club']['id'] in club_ids:
                fixtures_data.append({
                    ...
                    "realWorldTimestamp": pd.to_datetime(fixture['realWorldTimestamp'], unit='ms', utc=True),
                    ...
                    "position": next((ct['position'] for ct in fixture['tournament']['clubTournaments'] if ct['clubId'] == clubFixture['club']['id']), None)
                })
