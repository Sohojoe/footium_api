...
class GqlConnection:
    def __init__(self, url="https://live.api.footium.club/api/graphql", load_schema=None, report_strategy:ReportStrategy=LogReportStrategy()):
        self.url = url
    ...
    def send_query(self, query, variables=None, operation_name=None):
        gql_query = gql(query)
        response = self.client.execute(gql_query, variable_values=variables, operation_name=operation_name)
        boxed_response = Box(response)
        return boxed_response

    def send_paging_query(self, query, variables=None, operation_name=None, skip=0, take=20, stop=None):
        gql_query = gql(query)
        results = None
        count = 0
        while True:
            ...
            if len(list(boxed_response.keys())) != 1:
                raise NotImplementedError(f"send_paging_query only supports single key, got {len(list(boxed_response.keys()))}")
